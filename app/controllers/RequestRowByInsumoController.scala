...
class RequestRowByInsumoController @Inject()(repo: RequestRowRepository, repoRowCustomer: RequestRowCustomerRepository, repoProductReq: ProductRequestRepository, repoUnit: MeasureRepository,
  ...
  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.order.requestRowByInsumo_add(new MyDeadboltHandler, requestIdParm, searchProductForm, errorForm, productRequestsMap, products, unidades)))
      },
      res => {
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getById(id).map { res =>
      ...
      Ok(views.html.order.requestRowByInsumo_show(new MyDeadboltHandler, res(0), requestRowDrivers))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map {
      case (res) =>
        ...
        products = getProductsMap()
        updatedRow = res(0)
        Ok(views.html.order.requestRowByInsumo_update(new MyDeadboltHandler, updatedRow, updateForm.bind(anyData), productRequestsMap, products, unidades))
    }
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.order.requestRowByInsumo_update(new MyDeadboltHandler, updatedRow, errorForm, productRequestsMap, products, unidades)))
      },
      res => {
  ...
  def searchProductPost = LanguageAction.async { implicit request =>
    searchProductForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.order.requestRowByInsumo_add(new MyDeadboltHandler, requestIdParm, searchProductForm, newForm, productRequestsMap, products, unidades)))
      },
      res => {
