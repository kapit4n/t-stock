...
class MeasureController @Inject()(repo: MeasureRepository, val messagesApi: MessagesApi)(implicit ec: ExecutionContext) extends Controller with I18nSupport {
  ...
  def addGet = LanguageAction { implicit request =>
    ...
    Ok(views.html.measure.measure_add(new MyDeadboltHandler, newForm, measures))
  }

  def index = LanguageAction.async { implicit request =>
    repo.list().map { res =>
      Ok(views.html.measure.measure_index(new MyDeadboltHandler, res))
    }
  }
  ...
  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.measure.measure_add(new MyDeadboltHandler, errorForm, measures)))
      },
      res => {
        repo.create(
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      Ok(views.html.measure.measure_show(new MyDeadboltHandler, res(0)))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getById(id).map { res =>
      ...
      Ok(views.html.measure.measure_update(new MyDeadboltHandler, updatedRow, updateForm.bind(anyData), measures))
    }
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.measure.measure_update(new MyDeadboltHandler, updatedRow, errorForm, measures)))
      },
      res => {
        repo.update(
