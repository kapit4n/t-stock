...
class CustomerController @Inject() (
  ...
  def addGet() = LanguageAction.async { implicit request =>
    repo.list(0 * interval, interval).map { res =>
      Ok(views.html.customer.customer_add(new MyDeadboltHandler, newForm))
    }
  }
  ...
  def index(start: Int) = LanguageAction.async { implicit request =>
    if (start == 0) {
      Future(Ok(views.html.customer.customer_index(new MyDeadboltHandler, newForm, searchForm, customers, total, currentPage, interval)))
    } else {
      repo.list((start - 1) * interval, interval).map { res =>
        customers = res
        total = getTotal()
        currentPage = start
        Ok(views.html.customer.customer_index(new MyDeadboltHandler, newForm, searchForm, customers, total, currentPage, interval))
      }
    }
  }
  ...
  def index_company() = LanguageAction.async { implicit request =>
    repo.listCompany().map { res =>
      companys = res
      Ok(views.html.company.company_index(new MyDeadboltHandler, companys))
    }
  }
  ...
  def searchCustomerPost = LanguageAction.async { implicit request =>
    ...
    searchForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.customer.customer_index(new MyDeadboltHandler, newForm, searchForm, customers, total, currentPage, interval)))
      },
      res => {
  ...
  def index_pdf = LanguageAction {
    ...
    Ok(generator.toBytes(views.html.report.reporte_customeres(), "http://localhost:9000/")).as("application/pdf")
  }
  ...
  def add = LanguageAction.async { implicit request =>
    ...
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.customer.customer_add(new MyDeadboltHandler, errorForm)))
      },
      res => {
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getById(id).map { res =>
      Ok(views.html.customer.customer_show(new MyDeadboltHandler, res(0), requests, discounts))
    }
  }
  ...
  def showCompany(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getCompanyById(id).map { res =>
      Ok(views.html.company.company_show(new MyDeadboltHandler, res(0), customeresByAsso))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      ...
      Ok(views.html.customer.customer_update(new MyDeadboltHandler, updatedRow, updateForm.bind(anyData)))
    }
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.customer.customer_update(new MyDeadboltHandler, updatedRow, errorForm)))
      },
      res => {
        repo.update(
