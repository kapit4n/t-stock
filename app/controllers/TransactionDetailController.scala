...
class TransactionDetailController @Inject() (repo: TransactionDetailRepository, repoTransaction: TransactionRepository,
  ...
  def index = LanguageAction {
    Ok(views.html.transaction.transactionDetail_index())
  }

  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.transaction.transactionDetail_add(new MyDeadboltHandler, parentId, errorForm, transactionMap, accountMap)))
      },
      res => {
        repo.create(
  ...
  def addGet(transactionId: Long) = LanguageAction { implicit request =>
    ...
    Ok(views.html.transaction.transactionDetail_add(new MyDeadboltHandler, parentId, newForm, transactionMap, accountMap))
  }
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      Ok(views.html.transaction.transactionDetail_show(new MyDeadboltHandler, res(0)))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map {
      case (res) =>
        ...
        accountMap = getAccountMap(parenType)
        Ok(views.html.transaction.transactionDetail_update(new MyDeadboltHandler, updatedRow, updateForm.bind(anyData), accountMap))
    }
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.transaction.transactionDetail_update(new MyDeadboltHandler, updatedRow, errorForm, accountMap)))
      },
      res => {
        repo.update(
