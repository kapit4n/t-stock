...
class ProductTransformController @Inject() (repo: ProductInvRepository, repoProduct: ProductRepository, repoProductVendor: ProductVendorRepository,
  ...
  val newForm: Form[CreateProductTransformForm] = Form {
    mapping(
      "productId" -> longNumber,
      "vendorId" -> longNumber,
      "measureId" -> longNumber,
      "amount" -> number)(CreateProductTransformForm.apply)(CreateProductTransformForm.unapply)
  }

  // update required
  val updateForm: Form[UpdateProductTransformForm] = Form {
    mapping(
      "id" -> longNumber,
      "productId" -> longNumber,
      "vendorId" -> longNumber,
      "measureId" -> longNumber,
      "amount" -> number,
      "amountLeft" -> number)(UpdateProductTransformForm.apply)(UpdateProductTransformForm.unapply)
  }
  ...
  def index = LanguageAction.async { implicit request =>
    repo.list().map { res =>
      Ok(views.html.product.productTransform_list(new MyDeadboltHandler, res))
    }
  }
  ...
  def addGet(productId: Long) = LanguageAction { implicit request =>
    ...
    Ok(views.html.product.productTransform_add(new MyDeadboltHandler, productId, newForm, productMap, vendorMap, measureMap))
  }

  def getProductTransforms = LanguageAction.async {
    repo.list().map { res =>
      Ok(Json.toJson(res))
    }
  }
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      Ok(views.html.product.productTransform_show(new MyDeadboltHandler, res(0)))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map {
      case (res) =>
        ...
        measureMap = getMeasureMap(productMeasureId)
        Ok(views.html.product.productTransform_update(new MyDeadboltHandler, updatedRow, updateForm.bind(anyData), productMap, vendorMap, measureMap))
    }
  }
  ...
  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.product.productTransform_add(new MyDeadboltHandler, productId, errorForm, productMap, vendorMap, measureMap)))
      },
      res => {
        repo.create(
          ...
          res.amount, res.amount).map { resNew =>
            ...
            Redirect(routes.ProductTransformController.show(resNew.id))
          }
      })
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.product.productTransform_update(new MyDeadboltHandler, updatedRow, updateForm, productMap, vendorMap, measureMap)))
      },
      res => {
        ...
        repo.update(
          ...
          res.amount, res.amountLeft).map { _ =>
            ...
            Redirect(routes.ProductTransformController.show(res.id))
          }
      })
  }
...
case class CreateProductTransformForm(productId: Long, vendorId: Long, measureId: Long, amount: Int)

case class UpdateProductTransformForm(id: Long, productId: Long, vendorId: Long, measureId: Long, amount: Int, amountLeft: Int)