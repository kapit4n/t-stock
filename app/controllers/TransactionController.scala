...
class TransactionController @Inject()(
  ...
  def index = LanguageAction.async { implicit request =>
    repo.list().map { res =>
      Ok(views.html.transaction.transaction_index(new MyDeadboltHandler, res))
    }
  }
  ...
  def addGet(type_t: Long) = LanguageAction { implicit request =>
    users = getUsersMap()
    if (type_t == 1) { // Income
      Ok(views.html.transaction.transaction_add_1(new MyDeadboltHandler, newFormIncome))
    } else {
      Ok(views.html.transaction.transaction_add_2(new MyDeadboltHandler, newFormOutcome, users))
    }
  }
  ...
  def addIncome = LanguageAction.async { implicit request =>
    newFormIncome.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.transaction.transaction_add_1(new MyDeadboltHandler, errorForm)))
      },
      res => {
  ...
  def addOutcome = LanguageAction.async { implicit request =>
    newFormOutcome.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.transaction.transaction_add_2(new MyDeadboltHandler, errorForm, users)))
      },
      res => {
        var userId = request.session.get("userId").getOrElse("0").toLong
        var userName = request.session.get("userName").getOrElse("0").toString
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getById(id).map { res =>
      if (res(0).type_1 == "Income") {
        Ok(views.html.transaction.transaction_showIncome(new MyDeadboltHandler, res(0), details))
      } else {
        Ok(views.html.transaction.transaction_showOutcome(new MyDeadboltHandler, res(0), details))
      }
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getById(id).map { res =>
      ...
      if (updatedRow.type_1 == "Income") {
        ...
        Ok(views.html.transaction.transaction_updateIncome(new MyDeadboltHandler, updatedRow, updateFormIncome.bind(anyData)))
      } else {
        val anyData = Map(
          "id" -> id.toString().toString(),
          "date" -> updatedRow.date.toString(),
        ...
        Ok(views.html.transaction.transaction_updateOutcome(new MyDeadboltHandler, updatedRow, updateFormOutcome.bind(anyData), users))
      }
    }
  }
  ...
  def updatePostIncome = LanguageAction.async { implicit request =>
    updateFormIncome.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.transaction.transaction_updateIncome(new MyDeadboltHandler, updatedRow, errorForm)))
      },
      res => {
        repo.updateIncome(
  ...
  def updatePostOutcome = LanguageAction.async { implicit request =>
    updateFormOutcome.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.transaction.transaction_updateOutcome(new MyDeadboltHandler, updatedRow, errorForm, users)))
      },
      res => {
        repo.update(
          res.id, res.date, res.type_1,
