...
class ProductInvController @Inject() (repo: ProductInvRepository, repoProduct: ProductRepository, repoProductVendor: ProductVendorRepository,
  ...
  def index = LanguageAction.async { implicit request =>
    repo.list().map { res =>
      Ok(views.html.inventary.productInv_index(new MyDeadboltHandler, res))
    }
  }
  ...
  def addGet(productId: Long) = LanguageAction { implicit request =>
    ...
    Ok(views.html.inventary.productInv_add(new MyDeadboltHandler, productId, newForm, productMap, vendorMap, measureMap))
  }
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      Ok(views.html.inventary.productInv_show(new MyDeadboltHandler, res(0)))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map {
      case (res) =>
        ...
        measureMap = getMeasureMap(productMeasureId)
        Ok(views.html.inventary.productInv_update(new MyDeadboltHandler, updatedRow, updateForm.bind(anyData), productMap, vendorMap, measureMap))
    }
  }
  ...
  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.inventary.productInv_add(new MyDeadboltHandler, productId, errorForm, productMap, vendorMap, measureMap)))
      },
      res => {
        repo.create(
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.inventary.productInv_update(new MyDeadboltHandler, updatedRow, updateForm, productMap, vendorMap, measureMap)))
      },
      res => {
