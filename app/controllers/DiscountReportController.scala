...
class DiscountReportController @Inject() (repo: DiscountReportRepository, repoProd: CustomerRepository,
  ...
  def index = LanguageAction.async { implicit request =>
    repo.list().map { res =>
      Ok(views.html.discount.discountReport_index(new MyDeadboltHandler, res))
    }
  }
  ...
  def addGet = LanguageAction { implicit request =>
    ...
    Ok(views.html.discount.discountReport_add(new MyDeadboltHandler, newForm, customeresNames))
  }

  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.discount.discountReport_add(new MyDeadboltHandler, newForm, customeresNames)))
      },
      res => {
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getById(id).map { res =>
      Ok(views.html.discount.discountReport_show(new MyDeadboltHandler, res(0), children))
    }
  }
  ...
  def show_pdf(id: Long) = LanguageAction {
    ...
    Ok(generator.toBytes(views.html.discount.discountReport_show_pdf(values, discountReport.startDate, discountReport.endDate, discountReport.total.toString(), setting), "http://localhost:9000/")).as("application/pdf")
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      ...
      updatedRow = res(0)
      Ok(views.html.discount.discountReport_update(new MyDeadboltHandler, res(0), updateForm.bind(anyData), customeresMap))
    }
  }
  ...
  */
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.discount.discountReport_update(new MyDeadboltHandler, updatedRow, errorForm, Map[String, String]())))
      },
      res => {
