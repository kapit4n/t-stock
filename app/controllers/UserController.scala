...
class UserController @Inject() (
  ...
  def index = LanguageAction.async { implicit request =>
    repo.list().map { res =>
      Ok(views.html.user.user_index(new MyDeadboltHandler, res))
    }
  }

  def addGet = LanguageAction { implicit request =>
    Ok(views.html.user.user_add(new MyDeadboltHandler, newForm, types))
  }
  ...
  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.user.user_add(new MyDeadboltHandler, errorForm, types)))
      },
      res => {
        repo.create(
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    ...
    repo.getById(id).map { res =>
      Ok(views.html.user.user_show(new MyDeadboltHandler, res(0), roles, assignedRoles))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      ...
      Ok(views.html.user.user_update(new MyDeadboltHandler, updateRow, updateForm.bind(anyData), types))
    }
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.user.user_update(new MyDeadboltHandler, updateRow, errorForm, types)))
      },
      res => {
        repo.update(
