...
class VendorController @Inject() (repo: VendorRepository, val messagesApi: MessagesApi)(implicit ec: ExecutionContext) extends Controller with I18nSupport {
  ...
  def index = LanguageAction.async { implicit request =>
    repo.list().map { res =>
      Ok(views.html.vendor.vendor_index(new MyDeadboltHandler, res))
    }
  }

  def addGet = LanguageAction { implicit request =>
    Ok(views.html.vendor.vendor_add(new MyDeadboltHandler, newForm))
  }

  def addVendor = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.vendor.vendor_add(new MyDeadboltHandler, errorForm)))
      },
      vendor => {
        repo.create(
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      Ok(views.html.vendor.vendor_show(new MyDeadboltHandler, res(0)))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      ...
      Ok(views.html.vendor.vendor_update(new MyDeadboltHandler, updatedRow, updateForm.bind(anyData)))
    }
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.vendor.vendor_update(new MyDeadboltHandler, updatedRow, errorForm)))
      },
      res => {
        repo.update(
