...
class DiscountDetailController @Inject()(repo: DiscountDetailRepository, repoDiscReport: DiscountReportRepository,
  ...
  def index = LanguageAction {
    Ok(views.html.discount.discountDetail_index())
  }

  def add = LanguageAction.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.discount.discountDetail_add(new MyDeadboltHandler, discountId, errorForm, discountsNames, customers)))
      },
      res => {
  ...
  def searchCustomerPost = LanguageAction.async { implicit request =>
    searchCustomerForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.discount.discountDetail_add(new MyDeadboltHandler, discountId, newForm, discountsNames, customers)))
      },
      res => {
  ...
  def show(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      Ok(views.html.discount.discountDetail_show(new MyDeadboltHandler, res(0)))
    }
  }
  ...
  def getUpdate(id: Long) = LanguageAction.async { implicit request =>
    repo.getById(id).map { res =>
      ...
      udpatedRow = res(0)
      Ok(views.html.discount.discountDetail_update(new MyDeadboltHandler, udpatedRow, updateForm.bind(anyData), discountsNames, customers))
    }
  }
  ...
  def updatePost = LanguageAction.async { implicit request =>
    updateForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.discount.discountDetail_update(new MyDeadboltHandler, udpatedRow, updateForm, discountsNames, customers)))
      },
      res => {
