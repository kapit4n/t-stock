...
import models.ProductTransform
 ...
 */
@Singleton
class ProductTransformRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  private val dbConfig = dbConfigProvider.get[JdbcProfile]
  import dbConfig._
  import driver.api._

  private class ProductTransformTable(tag: Tag) extends Table[ProductTransform](tag, "productInv") {
    ...
    def * = (id, productId, productName, vendorId, vendorName, measureId, measureName, amount, amountLeft) <> ((ProductTransform.apply _).tupled, ProductTransform.unapply)
  }

  private val tableQ = TableQuery[ProductTransformTable]

  def create(productId: Long, productName: String, vendorId: Long, vendorName: String, measureId: Long, measureName: String, amount: Int, amountLeft: Int): Future[ProductTransform] = db.run {
    (tableQ.map(p => (
      p.productId, p.productName, p.vendorId,
      p.vendorName, p.measureId, p.measureName, p.amount, p.amountLeft))
      returning tableQ.map(_.id)
      into ((nameAge, id) => ProductTransform(
  ...
  def list(): Future[Seq[ProductTransform]] = db.run {
    tableQ.result
  }

  def listByInsumo(id: Long): Future[Seq[ProductTransform]] = db.run {
    tableQ.filter(_.productId === id).result
  }

  def listByProductId(id: Long): Future[Seq[ProductTransform]] = db.run {
    tableQ.filter(_.productId === id).result
  }
  ...
  // to cpy
  def getById(id: Long): Future[Seq[ProductTransform]] = db.run {
    tableQ.filter(_.id === id).result
  }

  // update required to copy
  def update(id: Long, productId: Long, productName: String, vendorId: Long, vendorName: String, measureId: Long, measureName: String, amount: Int, amountLeft: Int): Future[Seq[ProductTransform]] = db.run {
  ...
  // delete required
  def delete(id: Long): Future[Seq[ProductTransform]] = db.run {
  ...
  def updateProductTransform(productId: Long, quantity: Int) = {
  ...
  def udpateAmountLeft(pInv: ProductTransform, quantity: Int) = {
